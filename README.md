...
A comprehensive, production-ready URL Shortener service built with Spring Boot. This application allows users to convert long URLs into short, easy-to-share links and tracks click analytics for each link.
...
| ------ | ---------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `POST` | `/url/shorten`         | Creates a new short link. `customAlias` and `hoursToExpire` are optional. | `{"url": "...", "customAlias": "...", "hoursToExpire": 24}` |
| `GET`  | `/{shortCode}`         | Redirects to the original URL.                                            | N/A                                                           |
| `GET`  | `/url/stats/{shortCode}` | Retrieves click statistics for a short link.                              | N/A                                                           |
